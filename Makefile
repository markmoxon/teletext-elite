...
all:
	...
	$(BEEBASM) -i 1-source-files/main-sources/elite-disc.asm -do 5-compiled-game-discs/teletext-elite$(suffix).ssd -opt 3 -title "E L I T E"
ifneq ($(verify), no)
...
b2:
	...
	curl -H "Content-Type:application/binary" --upload-file "5-compiled-game-discs/teletext-elite$(suffix).ssd" "http://localhost:48075/run/b2?name=teletext-elite$(suffix).ssd"
